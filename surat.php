<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['act'])){
            ...
            switch ($act) {
                case 'add':
                    include "buat_surat.php";
                    break;
                case 'edit':
                    include "buat_surat_edit.php";
                    break;
                case 'print':
                    include "buat_surat_print.php";
                    break;
            }
        } else {
                        ...
                        <div class="z-depth-1">
                            <nav class="secondary-nav">
                                <div class="nav-wrapper blue-grey darken-1">
                                    <div class="col m7">
                                        <ul class="left">
                                            <li class="waves-effect waves-light hide-on-small-only"><a href="?page=surat" class="judul"><i class="material-icons">drafts</i> Surat Keluar</a></li>
                                            <li class="waves-effect waves-light">
                                                <a href="?page=surat&act=add"><i class="material-icons md-24">add_circle</i> Buat Surat</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                ...
                <?php
                    if(isset($_REQUEST['submit'])){
                        ...
                        <div class="col m12" id="colres">
                            <table class="bordered" id="tbl">
                                <thead class="blue lighten-4" id="head">
                                    <tr>
                                        <th width="10%">No</th>
                                        <th width="31%">Isi Ringkas<br/> Perihal</th>
                                ...
                                $query = mysqli_query($config, "SELECT * FROM tbl_buat_surat WHERE kegiatan LIKE '%$cari%' ORDER by id DESC LIMIT $curr, 15");
                                      ...
                                      echo '<tr><td>';
                                        ...
                                        echo $row['no_surat'].'<br/><hr/>'.indoDate($row['tgl_surat']).'</td><td>';
                                            ...
                                            echo '<a class="btn small deep-orange waves-effect waves-light" href="?page=surat&act=edit&id='.$row['id'].'">
                                                    ...
                                                    <a class="btn small blue darken-3 waves-effect waves-light" href="?page=surat&act=print&id='.$row['id'].'">
                                                    <i class="material-icons">print</i> PRINT</a></td></tr>';
                                        $no++;
                                    }
                                } else {
                        ...
                        <table class="bordered" id="tbl">
                            ...
                            //script untuk mencari data
                            $query = mysqli_query($config, "SELECT * FROM tbl_buat_surat ORDER by id DESC LIMIT $curr, 15");
                                if(mysqli_num_rows($query) > 0){
                                    $no = 1;
                                    while($row = mysqli_fetch_array($query)){
                                      echo '
                    ...
                    $query = mysqli_query($config, "SELECT * FROM tbl_buat_surat");
